...
func _on_reaction_triggered(reaction_type: int, elements: Array, target):
	...
	var reaction_damage = calculate_reaction_damage(base_damage, reaction_type, total_stacks / elements.size()) if elements.size() > 0 else 0.0
	...
	# 应用反应伤害到目标
	if reaction_damage > 0.0 and is_instance_valid(target) and target.has_method("take_damage"):
		target.take_damage(reaction_damage)
		EventBus.combat.damage_dealt.emit(self, target, reaction_damage, false)

	GameLogger.debug("元素反应触发", {"reaction": get_reaction_name(reaction_type), "damage": reaction_damage})
