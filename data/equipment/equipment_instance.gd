...
func can_wear(player) -> bool:
	...
	for attr in wear_requirements:
		...
		if attr == "境界":
			...
			if player_realm_level < required:
				return false
		# 技能等级需求
		elif attr == "技能等级":
			if typeof(required) == TYPE_DICTIONARY:
				for skill_name in required.keys():
					...
					var player_skill_level = player.get_skill_level(skill_name) if player.has_method("get_skill_level") else 0
...
func get_requirement_text(player = null) -> String:
	...
	for attr in wear_requirements:
		...
		if attr == "境界":
			...
			texts.append({"name": "境界", "required": required, "met": met})
		elif attr == "技能等级":
			if typeof(required) == TYPE_DICTIONARY:
				for skill_name in required.keys():
					var player_skill_level = player.get_skill_level(skill_name) if player and player.has_method("get_skill_level") else 0
					met = player_skill_level >= required[skill_name]
					texts.append({"name": skill_name + "等级", "required": required[skill_name], "met": met})
		else:
