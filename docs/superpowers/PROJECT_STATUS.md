...
| 文件 | 说明 |
|------|------|
| `data/factions/faction_data.gd` | 四大阵营定义 |
| `data/factions/faction_quest_data.gd` | 势力任务(24个) |
| `data/factions/faction_narrative_data.gd` | 势力剧情文本 |
| `data/factions/faction_unique_equipment.gd` | 势力专属装备 |
| `systems/factions/faction_system.gd` | 势力声望管理 |
| `scenes/ui/faction_panel.gd` | 阵营UI面板 |

**内容：**
- 四大阵营：星火殿(红金)/寒霜阁(蓝白)/机魂教(铜橙)/守墓人(黑银)
- 3友好+1敌对：可加入/击杀获得徽记
- 声望等级：陌生→友善→信任→尊敬→崇拜→狂热
- 声望折扣：最高8折
- 势力商店兑换

### 18. 主线/支线任务系统 ✅ (Phase 3)
| 文件 | 说明 |
|------|------|
| `data/quests/quest_data.gd` | 任务数据 |
| `systems/quests/quest_system.gd` | 任务跟踪管理 |
| `scenes/ui/quest_panel.gd` | 任务UI面板 |

**内容：**
- 序章+4章主线任务(共10个)
- 7个支线任务(收集/击杀)
- 任务状态：锁定→可接取→进行中→已完成→已领取
- 奖励：星尘、记忆碎片

### 19. 唯一性装备系统 ✅ (Phase 3)
| 文件 | 说明 |
|------|------|
| `data/equipment/unique_equipment_data.gd` | 21件唯一装备 |
| `data/equipment/equipment_instance.gd` | 唯一装备实例化 |

**内容：**
- 21件唯一装备：武器9件/护甲6件/饰品6件
- 稀有度：橙色(11件) + 红色(10件)
- 每件有独特词缀组合+背景故事+lore
- 按区域等级/境界解锁
...
| 文件 | 说明 |
|------|------|
| `data/achievements/achievement_data.gd` | 成就定义 |
| `systems/achievements/achievement_system.gd` | 成就跟踪 |
| `scenes/ui/achievement_panel.gd` | 成就UI面板 |
...
| 商店场景 | `scenes/ui/shop_panel.tscn` | 场景缺少TabsContainer等节点，补全完整UI结构 |
| 地图节点点击 | `scenes/map/node_scene.tscn` | `mouse_filter=2`→`1`，修复节点点击无反应问题 |
| 地图信号连接 | `scenes/map/map_scene.gd` | 信号连接改用`call_deferred`，确保节点初始化后连接 |
...
### Phase 3 后续
1. **内部Playtest** - 验证战斗节奏、Build多样性、任务流程
2. **数值平衡微调** - 根据测试反馈调整
3. **UI细节完善** - 各面板交互优化
...
*项目状态：Phase 1-3 开发完成，核心内容齐全，等待全面测试*
